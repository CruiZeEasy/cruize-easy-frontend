...
import { RentType } from "@/constants/enums";
import clsx from "clsx";
import { AnimatePresence, motion } from "framer-motion";

interface RentTypeDisplayProps {
  rentType: RentType;
}

export function RentTypeDisplay({ rentType }: RentTypeDisplayProps) {
  const isSelfDrive = rentType === "SELF_DRIVE";
  ...
  return (
    <div className="text-sm">
      ...
      <div className="grid grid-cols-2 gap-4 mt-2">
        <div
          className={clsx(
            "py-3 px-4 rounded-lg font-gilroy-medium  transition-all",
            isSelfDrive
              ? "bg-primary text-white"
              : "bg-white outline outline-primary-dark  text-primary-dark hover:border-primary-dark"
          )}
        ...
        <div
          className={clsx(
            "py-3 px-4 rounded-lg font-gilroy-medium  transition-all cursor-default",
            !isSelfDrive
              ? "bg-primary text-white"
              : "bg-white outline outline-primary-dark  text-primary-dark hover:border-primary-dark"
          )}
