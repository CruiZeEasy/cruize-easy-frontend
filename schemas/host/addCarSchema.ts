...
export const pickupLocationSchema = z.object({
  ...
  pickupLatitude: z.number().min(-90).max(90, "Invalid latitude"),
  pickupLongitude: z.number().min(-180).max(180, "Invalid longitude"),
...
// Combined schema with pickup location
export const addCarSchema = z.object({
  ...vehicleInformationSchema.shape,
  ...vehicleLicensesSchema.shape,
  ...rentInformationSchema.shape,
  ...pickupLocationSchema.shape, // NEW: Added location fields
  ...carImagesSchema.shape,
  ...workingHoursSchema.shape,
});
