...
import { RentTypeDisplay } from "@/components/dashboard/listings/RentTypeDisplay";
...
const bookingSchema = z
  ...
  .refine(
    (data) => {
      // If dropoffSameAsPickup is false, require dropoff address and coordinates
      if (!data.dropoffSameAsPickup) {
        return (
          data.dropoffAddress &&
          data.dropoffAddress.trim().length > 0 &&
    ...
    {
      message:
        "Please select a dropoff location or check 'Same as pickup location'",
      path: ["dropoffAddress"],
    }
  );
...
export default function BookingDetailsPage() {
  ...
  const onSubmit = (data: BookingFormData) => {
    ...
    const pickupLocation = vehicleDetails.pickupLocation;
      ...
      dropOffLocation: data.dropoffSameAsPickup
        ? pickupLocation
          ? {
              ...
              coordinates: {
                x: pickupLocation.longitude || 0,
                y: pickupLocation.latitude || 0,
                type: "Point" as const,
                coordinates: [
  ...
  const vehicleName = `${vehicleDetails.brand} ${vehicleDetails.name}`;
  const workingHours = vehicleDetails.workingHours || [];
  const pickupLocation = vehicleDetails.pickupLocation;
        ...
        <div className="px-4 lg:px-12 lg:py-4 mt-8 lg:mt-10">
          <section className="grid lg:grid-cols-2 gap-x-8">
            ...
            <div className="space-y-6">
              <div className="space-y-4 bg-white md:p-4 rounded-[20px]">
                <div className="relative w-full h-[278px]">
                  <Image
                    src={primaryImage || "/images/cars/1.webp"}
                    fill
                    alt={vehicleName}
                    quality={100}
                  ...
                  {showWorkingHours && (
                    <div className="mt-4 space-y-3 pt-4 border-t border-neutral-275">
                      {workingHours.map((schedule) => (
                        ...
                        >
                          <span className="text-neutral-430">
                            {schedule.day}
                          </span>
                          {schedule.isActive ? (
            ...
            <div className="bg-white rounded-[20px] mt-6 lg:mt-0 md:p-4 flex flex-col justify-between">
              <RentTypeDisplay rentType={vehicleDetails.rentType} />
